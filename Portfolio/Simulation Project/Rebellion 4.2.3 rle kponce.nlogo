...
to setup
  ...
  ; set globals
  set k 2.3
  set threshold random-float 1.5

  ask patches [
    ; make background a slightly dark gray
    set pcolor gray - 1
    ; cache patch neighborhoods
    set neighborhood patches in-radius cop-power
  ]

  if fear-percent + generous-percent > 100 [
    user-message (word
      "The sum of fear-percent and generous-percent "
  ...
  if fear-percent + generous-percent = 100 [
    user-message (word
      "The sum of fear-percente and generous-percent "
      "should not be equal to 100.")
    stop
  ]

  ; create cops
  create-cops round (fear-percent * .01 * count patches) [
  ...
  ; create agents
  create-agents round (generous-percent * .01 * count patches) [
    move-to one-of patches with [ not any? turtles-here ]
    set heading 0
...
to determine-behavior
  set active? (grievance - risk-aversion * estimated-arrest-probability + taxes - manipulate  > threshold)
end

to-report grievance
  report perceived-hardship * (1 - leader-reputation)
end
...
to execute-prisoners
  ask agents with [ jail-term > jail-time / 5 ] [ die ]
  set threshold threshold + 0.001
end
...
to enforce
  if any? (agents-on neighborhood) with [ active? ] [
    ...
    ask suspect [
      set active? false
      set jail-term random jail-time
    ]
  ]
end
...
304
72
The first two sliders can not equal or exceed 100%.
11
0.0
1
...
265
198
Fear percent and Generous percent affect this!
11
0.0
1
...
369
149
If you change the first 3 sliders while the simulation is running, you will have to click setup again for it to have any effect
11
0.0
1
